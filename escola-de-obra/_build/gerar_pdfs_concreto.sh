#!/usr/bin/env bash
# Gera todos os PDFs do Kit do Concreto (Curso 3) — Chromium headless, offline.
set -euo pipefail
DIR="$(cd "$(dirname "$0")" && pwd)"
K="$DIR/../05-curso-concreto/kit"
H2P="$DIR/html2pdf.sh"

# E-book: markdown -> HTML diagramado
python3 - "$K/ebook/historias-concreto.md" "$K/ebook/historias-concreto.html" <<'PY'
import markdown, sys
...
html=f"""<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>Histórias Reais de Obra — Concreto</title><style>{CSS}</style></head><body>
<div class="cover"><div class="lz"><span>GD</span></div><h1>Histórias Reais<br>de Obra</h1><div class="sub">Concreto — 10 relatos baseados em situações reais recorrentes</div>
...
bash "$H2P" "$K/checklist/checklist-concreto.html" "$K/checklist/checklist-concreto-a4.pdf"
sed 's/size:A4/size:A5/' "$K/checklist/checklist-concreto.html" > /tmp/ckc-a5.html
bash "$H2P" /tmp/ckc-a5.html "$K/checklist/checklist-concreto-a5.pdf"
bash "$H2P" "$K/guia-bolso/guia-bolso-concreto.html" "$K/guia-bolso/guia-bolso-concreto.pdf"
bash "$H2P" "$K/ebook/historias-concreto.html" "$K/ebook/historias-concreto.pdf"
bash "$H2P" "$K/apostila/apostila-concreto.html" "$K/apostila/apostila-concreto.pdf"
bash "$H2P" "$K/relatorio/registro-concretagem.html" "$K/relatorio/registro-concretagem.pdf"
echo "== PDFs do Kit do Concreto gerados =="
echo "(fluxograma: SVG via mermaid-cli + wrapper — ver RELATORIO-QA)"
