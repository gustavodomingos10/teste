...
<div class="app">
  ...
  <footer>
    Ferramenta educativa — apoia a inspeção de liberação (NBR 14931:2023 · 6118 · 15696). Não substitui o
...
const STEPS = [
  ...
  { key:"escoramento", titulo:"ESCORAMENTO — apoio, prumo, contraventamento (NBR 15696:2009)", ops:[
...
function renderResultado(veredito,criticos,pendencias,passo){
  ...
  const share = encodeURIComponent(`Liberador de Concretagem (Escola de Obra): veredito = ${lbl}. Inspecao NBR 14931:2023/6118/15696.`);
  ...
  html += `<div class="disc"><b>Aviso técnico:</b> apoio educativo à inspeção de liberação (NBR 14931:2023 · 6118 · 15696), com calibragem conservadora — na dúvida, <b>não libere</b> e escale ao responsável técnico.
...
const CENARIOS = [
  {foto:"Negativos da laje afundando quando alguém pisa; sem caranguejos",
   ...
   exp:"O negativo só trabalha em cima. Exige caranguejos + passarelas para o tráfego da concretagem (NBR 14931:2023)."},
  {foto:"Armadura da viga encostada no fundo da fôrma, sem espaçador",
   ...
   exp:"Cobrimento é proteção química (NBR 6118:2023, Tab. 7.2). Espaçador é o seguro mais barato contra patologia."},
  {foto:"Escoramento sem diagonais de contraventamento nas duas direções",
   q:"Veredito da inspeção?",
   ops:["Liberar, diagonais são opcionais","Travar: NC estrutural — contraventar antes de concretar (NBR 15696:2009)","Liberar com pendência para depois","Observar durante a concretagem"],c:1,
  ...
  {foto:"Fresta no fundo da viga que passa luz",
   ...
   exp:"Fuga de nata deixa pedra sem pasta (ninho). Teste da luz + vedação na véspera (NBR 14931:2023)."},
  ...
  {foto:"Escora apoiada em bloco cerâmico deitado, no meio do vão",
   ...
   exp:"Apoio de escora exige base firme e distribuição (NBR 15696:2009). Bloco cerâmico esmaga/cede sob carga."},
