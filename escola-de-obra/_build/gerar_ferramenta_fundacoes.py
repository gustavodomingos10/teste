...
A escolha e o dimensionamento finais são SEMPRE do projetista de fundações (NBR 6122)."""
import os
...
<div class="app">
  ...
  <footer>
    ...
    dimensionamento finais são do projetista de fundações/geotécnico (NBR 6122).<br>
...
function recomendar(){
  ...
  if(sondagem==="nao" || camada==="naosei"){
    ...
    passo={t:"Primeiro passo obrigatório", d:"Exigir a sondagem de simples reconhecimento (SPT, NBR 6484) antes de definir a fundação. Sem ela, qualquer escolha é aposta — e o risco não é seu para assumir."};
    return render({familia,tipos:[],alertas:["Sem sondagem não há decisão técnica de fundação (NBR 6484)."],passo,exigir,warn});
  }
  ...
  passo={t:"Próximo passo", d:"Leve a família e os tipos candidatos ao projetista de fundações para definição e dimensionamento (NBR 6122). Registre solo (sondagem), cargas e restrições no parecer do Kit."};
...
function renderResultado(familia,tipos,alertas,passo,exigir,warn){
  ...
  const shareTxt = encodeURIComponent(`Seletor de Fundacao (Escola de Obra): familia recomendada = ${familia}. Decisao final do projetista de fundacoes (NBR 6122).`);
...
const CENARIOS = [
  ...
  {foto:"Não há boletim de sondagem no início da obra",
   ...
   ops:["Escolher sapata por ser mais barata","Exigir sondagem SPT (NBR 6484)","Cravar estaca de teste no olho","Copiar a obra vizinha"],c:1,
   exp:"Sem sondagem não há decisão técnica. A primeira providência é exigir a sondagem de simples reconhecimento (NBR 6484)."},
